...
#### 获取天地图API密钥
1. 访问 [天地图开发者控制台](https://console.tianditu.gov.cn/api/key)
2. 注册并登录国家地理信息公共服务平台
3. 创建新应用，选择"浏览器端"
4. 获取生成的"tk" (API Key)
5. 在应用设置中粘贴该密钥
...
```
1. 点击游玩点右侧的"🗺️"按钮
2. 根据设置选择的地图API（高德/Google/Bing/天地图）打开地图
3. 也可以单独为某个游玩点选择特定的地图服务
4. 高德地图：显示POI点详情，可进一步操作
5. Google/Bing/天地图：显示位置标记和周边信息
6. 纯查看模式，不进行导航规划
```
